...
<% try {
        ...
        if ("patient".equals(request.getParameter("action"))) {
            ...
            String country = request.getParameter("country") == "" ? "NA" : request.getParameter("country");
            String city = request.getParameter("city") == "" ? "NA" : request.getParameter("city");
            String maritalstatus = request.getParameter("maritalstatus") == "" ? "NA" : request.getParameter("maritalstatus");
            ...
            String nhismembershipid = request.getParameter("nhismembershipid") == "" ? "NA" : request.getParameter("nhismembershipid");
            int spid = 0;
        ...
        String dob = year+"-"+month+"-"+day;
            ...
            if(sponsorType == "nhis"){
               sd = mgr.createPatientSponsorshipDetails(p.getPatientid(), nhismembershipid, sponsorType, "NA", 0); 
            }
            if(sponsorType != "private"){
                sd = mgr.createPatientSponsorshipDetails(p.getPatientid(), membershipID, sponsorType, benefitPlan, spid);  
            }
            if(sponsorType == "cooperate" || sponsorType == "cash"){
                sd = mgr.createPatientSponsorshipDetails(p.getPatientid(), "NA", sponsorType, "NA", 0);
            }
            if (f == null) {
