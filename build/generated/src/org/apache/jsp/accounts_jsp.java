package org.apache.jsp;

import javax.servlet.*;
import javax.servlet.http.*;
import javax.servlet.jsp.*;
import entities.*;
import java.util.List;
import java.util.Date;
import java.text.SimpleDateFormat;
import java.text.DateFormat;
import java.util.Date;
import java.util.List;
import entities.*;
import java.text.SimpleDateFormat;
import entities.HMSHelper;
import java.text.DateFormat;

public final class accounts_jsp extends org.apache.jasper.runtime.HttpJspBase
    implements org.apache.jasper.runtime.JspSourceDependent {

  private static final JspFactory _jspxFactory = JspFactory.getDefaultFactory();

  private static java.util.List<String> _jspx_dependants;

  static {
    _jspx_dependants = new java.util.ArrayList<String>(1);
    _jspx_dependants.add("/widgets/leftsidebar.jsp");
  }

  private org.glassfish.jsp.api.ResourceInjector _jspx_resourceInjector;

  public java.util.List<String> getDependants() {
    return _jspx_dependants;
  }

  public void _jspService(HttpServletRequest request, HttpServletResponse response)
        throws java.io.IOException, ServletException {

    PageContext pageContext = null;
    HttpSession session = null;
    ServletContext application = null;
    ServletConfig config = null;
    JspWriter out = null;
    Object page = this;
    JspWriter _jspx_out = null;
    PageContext _jspx_page_context = null;

    try {
      response.setContentType("text/html;charset=UTF-8");
      pageContext = _jspxFactory.getPageContext(this, request, response,
      			null, true, 8192, true);
      _jspx_page_context = pageContext;
      application = pageContext.getServletContext();
      config = pageContext.getServletConfig();
      session = pageContext.getSession();
      out = pageContext.getOut();
      _jspx_out = out;
      _jspx_resourceInjector = (org.glassfish.jsp.api.ResourceInjector) application.getAttribute("com.sun.appserv.jsp.resource.injector");

      out.write("\n");
      out.write("\n");
      out.write("\n");
      out.write("\n");
      out.write("<!DOCTYPE html>\n");
      out.write("<html>\n");
      out.write("    <head>\n");
      out.write("        <meta charset=\"utf-8\">\n");
      out.write("        <title>ClaimSync Extended</title>\n");
      out.write("        <meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\">\n");
      out.write("        <meta name=\"description\" content=\"\">\n");
      out.write("        <meta name=\"author\" content=\"\">\n");
      out.write("\n");
      out.write("        <!-- Le HTML5 shim, for IE6-8 support of HTML elements -->\n");
      out.write("        <!--[if lt IE 9]>\n");
      out.write("        <script src=\"http://html5shim.googlecode.com/svn/trunk/html5.js\"></script>\n");
      out.write("        <![endif]-->\n");
      out.write("\n");
      out.write("        <!-- Le styles -->\n");
      out.write("        <link href=\"css/bootstrap.min.css\" rel=\"stylesheet\">\n");
      out.write("        <link href=\"css/bootstrap-responsive.css\" rel=\"stylesheet\">\n");
      out.write("        <link type=\"text/css\" href=\"css/custom-theme/jquery-ui-1.8.16.custom.css\" rel=\"stylesheet\" />\n");
      out.write("        <link href=\"css/docs.css\" rel=\"stylesheet\">\n");
      out.write("        <link rel=\"stylesheet\" href=\"css/styles.css\">\n");
      out.write("        <style type=\"text/css\" title=\"currentStyle\">\n");
      out.write("            @import \"css/jquery.dataTables_themeroller.css\";\n");
      out.write("            @import \"css/custom-theme/jquery-ui-1.8.16.custom.css\";\n");
      out.write("            body {\n");
      out.write("                /*    overflow: hidden; */\n");
      out.write("            }\n");
      out.write("\n");
      out.write("            .large_button {\n");
      out.write("                background-color: #35AFE3;\n");
      out.write("                background-image: -moz-linear-gradient(center top , #45C7EB, #2698DB);\n");
      out.write("                box-shadow: 0 1px 0 0 #6AD2EF inset;\n");
      out.write("                color: #FFFFFF;\n");
      out.write("                text-decoration: none;\n");
      out.write("                font-weight: lighter;\n");
      out.write("                font-family: \"Helvetica Neue\", Helvetica, Arial, sans-serif;\n");
      out.write("                font-size: 36px;\n");
      out.write("            }\n");
      out.write("\n");
      out.write("            .large_button:hover {\n");
      out.write("                background-color: #FBFBFB;\n");
      out.write("                background-image: -moz-linear-gradient(center top , #FFFFFF, #F5F5F5);\n");
      out.write("                background-repeat: repeat-x;\n");
      out.write("                border: 1px solid #DDDDDD;\n");
      out.write("                border-radius: 3px 3px 3px 3px;\n");
      out.write("                box-shadow: 0 1px 0 #FFFFFF inset;\n");
      out.write("                list-style: none outside none;\n");
      out.write("                color: #777777;\n");
      out.write("                /* padding: 7px 14px; */\n");
      out.write("            }\n");
      out.write("        </style>\n");
      out.write("\n");
      out.write("        <link href=\"css/tablecloth.css\" rel=\"stylesheet\" type=\"text/css\" media=\"screen\" />\n");
      out.write("\n");
      out.write("        ");

            HMSHelper mgr = new HMSHelper();
            DateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
            //Patient p = (Patient)session.getAttribute("patient");
            //get current date time with Date()
            Date date = new Date();
            //System.out.println(dateFormat.format(date));
            List visits = mgr.listUnitVisitations("Accounts", dateFormat.format(date));
            List treatments = null;
            // for (int i = 0; i < visits.size(); i++) {
            //   Visitationtable visit = (Visitationtable) visits.get(i);
        
      out.write("\n");
      out.write("\n");
      out.write("        <script type=\"text/javascript\">\n");
      out.write("            \n");
      out.write("            function alert(){\n");
      out.write("                \n");
      out.write("            }\n");
      out.write("        </script>\n");
      out.write("    </head>\n");
      out.write("\n");
      out.write("    <body data-spy=\"scroll\" data-target=\".subnav\" data-offset=\"50\">\n");
      out.write("\n");
      out.write("        <!-- Navbar\n");
      out.write("        ================================================== -->\n");
      out.write("        <div style=\"display: none;\" class=\"navbar navbar-fixed-top\">\n");
      out.write("            <div class=\"navbar-inner\">\n");
      out.write("                <div class=\"container\">\n");
      out.write("                    <a class=\"btn btn-navbar\" data-toggle=\"collapse\" data-target=\".nav-collapse\"> <span class=\"icon-bar\"></span> <span class=\"icon-bar\"></span> <span class=\"icon-bar\"></span> </a>\n");
      out.write("                    <a class=\"brand\" href=\"../\"><img src=\"images/logo.png\" width=\"200px;\" /></a>\n");
      out.write("\n");
      out.write("                    <div style=\"margin-top: 10px;\" class=\"nav-collapse\">\n");
      out.write("                        <ul class=\"nav pull-right\">\n");
      out.write("\n");
      out.write("                            <li class=\"dropdown\">\n");
      out.write("                                <a class=\"active\" > Logged in as:  Mr. Amoo </a>\n");
      out.write("\n");
      out.write("                            </li>\n");
      out.write("                            <li class=\"divider-vertical\"></li>\n");
      out.write("\n");
      out.write("                            <li class=\"dropdown\">\n");
      out.write("                                <a class=\"dropdown-toggle\" data-toggle=\"dropdown\" href=\"#\"><i class=\"icon-user\"></i> Account <b class=\"caret\"></b></a>\n");
      out.write("                                <ul class=\"dropdown-menu\">\n");
      out.write("\n");
      out.write("                                    <li>\n");
      out.write("                                        <a target=\"_blank\" href=\"bootstrap.min.css\"><i class=\"icon-wrench\"></i> Settings </a>\n");
      out.write("                                    </li>\n");
      out.write("\n");
      out.write("                                    <li>\n");
      out.write("                                        <a target=\"_blank\" href=\"bootstrap.css\"><i class=\"icon-question-sign\"></i> Help </a>\n");
      out.write("                                    </li>\n");
      out.write("                                    <li>\n");
      out.write("                                        <a target=\"_blank\" href=\"bootstrap.css\"><i class=\"icon-share\"></i> Feedback </a>\n");
      out.write("                                    </li>\n");
      out.write("                                    <li class=\"divider\"></li>\n");
      out.write("\n");
      out.write("                                    <li>\n");
      out.write("                                        <a target=\"_blank\" href=\"variables.less\"><i class=\"icon-off\"></i> Log Out</a>\n");
      out.write("                                    </li>\n");
      out.write("\n");
      out.write("                                </ul>\n");
      out.write("\n");
      out.write("                            </li>\n");
      out.write("                        </ul>\n");
      out.write("                    </div>\n");
      out.write("                </div>\n");
      out.write("            </div>\n");
      out.write("        </div>\n");
      out.write("\n");
      out.write("        <div class=\"container-fluid\">\n");
      out.write("\n");
      out.write("            <!-- Masthead\n");
      out.write("            ================================================== -->\n");
      out.write("            <header  class=\"jumbotron subhead\" id=\"overview\">\n");
      out.write("\n");
      out.write("                <div style=\"margin-top: 20px; margin-bottom: -50px;\" class=\"subnav navbar-fixed-top hide\">\n");
      out.write("                    <ul class=\"nav nav-pills\">\n");
      out.write("\n");
      out.write("                        <li>\n");
      out.write("                            <a href=\"#\">Pharmacy</a><span class=\"divider\"></span>\n");
      out.write("                        </li>\n");
      out.write("\n");
      out.write("                    </ul>\n");
      out.write("                </div>\n");
      out.write("\n");
      out.write("            </header>\n");
      out.write("\n");
      out.write("            <div style=\"position: absolute; left: 30%; top: 200px; text-align: center;\" class=\"progress1\">\n");
      out.write("                <img src=\"images/logoonly.png\" width=\"136px;\" style=\"margin-bottom: 20px;\" />\n");
      out.write("                <a href=\"#\"> <h3 class=\"segoe\" style=\"text-align: center; font-weight: lighter;\"> Your page is taking longer than expected to load.....\n");
      out.write("                        <br />\n");
      out.write("                        Please be patient!</h3>\n");
      out.write("                    <br />\n");
      out.write("                </a>\n");
      out.write("                <div  class=\"progress progress-striped active span5\">\n");
      out.write("\n");
      out.write("                    <div class=\"bar\"\n");
      out.write("                         style=\"width: 100%;\"></div>\n");
      out.write("                </div>\n");
      out.write("            </div>\n");
      out.write("\n");
      out.write("            <section style=\"margin-top: -30px;\" id=\"dashboard\">\n");
      out.write("\n");
      out.write("                <!-- Headings & Paragraph Copy -->\n");
      out.write("                <div class=\"row\">\n");
      out.write("\n");
      out.write("                    ");
      out.write("\n");
      out.write("\n");
      out.write("\n");
      out.write("\n");
      out.write("\n");
      out.write("\n");
HMSHelper mg = new HMSHelper();
    DateFormat dateformat = new SimpleDateFormat("yyyy-MM-dd");

    Date dt = new Date();
    //System.out.println(dateFormat.format(date));
    List vjs = mg.listRecentVisits(dateformat.format(dt), "In Patient");
    List outs = mg.listRecentVisits(dateformat.format(dt), "Out Patient");
    List labs = mg.listUnitVisitations("Laboratory", dateformat.format(dt));
    List pharmacy = mg.listUnitVisitations("Laboratory", dateformat.format(dt));
    List accs = mg.listUnitVisitations("Laboratory", dateformat.format(dt));
    List rms = mg.listUnitVisitations("Room 1", dateformat.format(dt));

      out.write("\n");
      out.write("\n");
      out.write("<div style=\"height: 100%; position: fixed;\" class=\"span3\">\n");
      out.write("    <ul style=\"display: none;\"  class=\"menu hide\">\n");
      out.write("        <li class=\"item1\">\n");
      out.write("            <a  href=\"#\"><i class=\"icon-folder-open\"></i> Patient Management</a>\n");
      out.write("            <ul>\n");
      out.write("                <li class=\"subitem1\">\n");
      out.write("                    <a href=\"index.jsp\">Front Desk </a>\n");
      out.write("                </li>\n");
      out.write("                <li class=\"subitem2\">\n");
      out.write("                    <a href=\"opd.jsp\">Out Patient Department </a>\n");
      out.write("                </li>\n");
      out.write("\n");
      out.write("            </ul>\n");
      out.write("\n");
      out.write("        </li>\n");
      out.write("        <li class=\"item2\">\n");
      out.write("            <a href=\"#\"> <i class=\" icon-search\"></i> Consultation\n");
      out.write("            </a> \n");
      out.write("            <ul>\n");
      out.write("                <li class=\"subitem1\">\n");
      out.write("                    <a href=\"consultingroom.jsp\">Patients <span>");
      out.print(rms.size());
      out.write("</span></a>\n");
      out.write("                </li>\n");
      out.write("                <li class=\"subitem2\">\n");
      out.write("                    <a href=\"#\">Appointments <span>6</span></a>\n");
      out.write("                </li>\n");
      out.write("                <li class=\"subitem3\">\n");
      out.write("                    <a href=\"outpatient.jsp\">Recent Out Patients <span>");
      out.print(vjs.size());
      out.write("</span></a>\n");
      out.write("                </li> \n");
      out.write("                <li class=\"subitem4\">\n");
      out.write("                    <a href=\"inpatient.jsp\">Recent In Patients <span>");
      out.print(outs.size());
      out.write("</span></a>\n");
      out.write("                </li> \n");
      out.write("                <li class=\"subitem5\">\n");
      out.write("                    <a href=\"#\">Lab Results<span>2</span></a>\n");
      out.write("                </li> \n");
      out.write("            </ul>\n");
      out.write("        </li>\n");
      out.write("        <li class=\"item2\">\n");
      out.write("            <a href=\"#\"> <i class=\" icon-search\"></i> Male Ward\n");
      out.write("            </a> \n");
      out.write("            <ul>\n");
      out.write("                <li class=\"subitem1\">\n");
      out.write("                    <a href=\"ward.jsp\">In Patients <span></span></a>\n");
      out.write("                </li>\n");
      out.write("                <li class=\"subitem2\">\n");
      out.write("                    <a href=\"#\">New Ward Report <span></span></a>\n");
      out.write("                </li>\n");
      out.write("                <li class=\"subitem3\">\n");
      out.write("                    <a href=\"#\">Ward Reports Previous Reports <span></span></a>\n");
      out.write("                </li> \n");
      out.write("                <li class=\"subitem4\">\n");
      out.write("                    <a href=\"#\">Schedule<span></span></a>\n");
      out.write("                </li> \n");
      out.write("\n");
      out.write("            </ul>\n");
      out.write("        </li>\n");
      out.write("        <li class=\"item3\">\n");
      out.write("            <a href=\"#\"><i class=\"icon-share\"></i> Laboratory  </a>\n");
      out.write("            <ul>\n");
      out.write("                <li class=\"subitem1\">\n");
      out.write("                    <a href=\"laboratory.jsp\">Patients </a><span>");
      out.print(labs.size());
      out.write("</span>\n");
      out.write("                </li>\n");
      out.write("                <li class=\"subitem2\">\n");
      out.write("\n");
      out.write("                </li>\n");
      out.write("                <li class=\"subitem3\">\n");
      out.write("                    <a href=\"\">Outstanding Treatments</a><span>6</span>\n");
      out.write("                </li>\n");
      out.write("                <li class=\"subitem3\">\n");
      out.write("                    <a href=\"#\">Procurement Sheet</a>\n");
      out.write("                </li>\n");
      out.write("\n");
      out.write("            </ul>\n");
      out.write("        </li>\n");
      out.write("        <li class=\"item4\">\n");
      out.write("            <a href=\"#\"><i class=\"icon-check\"></i> Pharmacy </a>\n");
      out.write("            <ul>\n");
      out.write("                <li class=\"subitem1\">\n");
      out.write("                    <a href=\"pharmacy.jsp\">Patients </a><span>");
      out.print(pharmacy.size());
      out.write("</span>\n");
      out.write("                </li>\n");
      out.write("                <li class=\"subitem2\">\n");
      out.write("\n");
      out.write("                </li>\n");
      out.write("                <li class=\"subitem3\">\n");
      out.write("                    <a href=\"\">Outstanding Treatments</a><span>6</span>\n");
      out.write("                </li>\n");
      out.write("                <li class=\"subitem3\">\n");
      out.write("                    <a href=\"\">Procurement Sheet</a>\n");
      out.write("                </li>\n");
      out.write("\n");
      out.write("            </ul>\n");
      out.write("        </li>\n");
      out.write("        <li class=\"item4\">\n");
      out.write("            <a href=\"#\"><i class=\"icon-check\"></i> Accounts and Billing</a>\n");
      out.write("            <ul>\n");
      out.write("                <li class=\"subitem1\">\n");
      out.write("                    <a href=\"accounts.jsp\">Current Bills </a><span>");
      out.print(accs.size());
      out.write("</span>\n");
      out.write("                </li>\n");
      out.write("\n");
      out.write("                <li class=\"subitem3\">\n");
      out.write("                    <a href=\"anballdepartments.jsp\">Departmental Bills</a>\n");
      out.write("                </li>\n");
      out.write("                <li class=\"subitem3\">\n");
      out.write("                    <a href=\"anballpatientsrevenues.jsp\">All Patients Revenues</a>\n");
      out.write("                </li>\n");
      out.write("                <li class=\"subitem3\">\n");
      out.write("                    <a href=\"anbpatientrevenues.jsp\">Patient Revenues</a>\n");
      out.write("                </li>\n");
      out.write("                <li class=\"subitem3\">\n");
      out.write("                    <a href=\"anballsponsorsrevenues.jsp\">Sponsors Revenues</a>\n");
      out.write("                </li>\n");
      out.write("\n");
      out.write("\n");
      out.write("            </ul>\n");
      out.write("        </li>\n");
      out.write("        <li class=\"item4\">\n");
      out.write("            <a href=\"#\"><i class=\"icon-check\"></i>Inventory</a>\n");
      out.write("            <ul>\n");
      out.write("                <li class=\"subitem1\">\n");
      out.write("                    <a href=\"inventoryindex.jsp\">Add Item</a>\n");
      out.write("                </li>\n");
      out.write("                <li class=\"subitem2\">\n");
      out.write("                    <a href=\"addType.jsp\">Add Item Type</a>\n");
      out.write("                </li>\n");
      out.write("                <li class=\"subitem3\">\n");
      out.write("                    <a href=\"search_item.jsp\">Search Item</a>\n");
      out.write("                </li>\n");
      out.write("                <li class=\"subitem4\">\n");
      out.write("                    <a href=\"application.jsp\">List Items</a>\n");
      out.write("                </li>\n");
      out.write("                <li class=\"subitem4\">\n");
      out.write("                    <a href=\"check.jsp\">Check Expiry</a>\n");
      out.write("                </li>\n");
      out.write("                <li class=\"subitem4\">\n");
      out.write("                    <a href=\"addSupplier.jsp\">Add Supplier</a>\n");
      out.write("                </li>\n");
      out.write("            </ul>\n");
      out.write("        </li>\n");
      out.write("        <li class=\"item4\">\n");
      out.write("            <a href=\"#\"><i class=\"icon-check\"></i>Admin Panel</a>\n");
      out.write("            <ul>\n");
      out.write("                <li class=\"subitem1\">\n");
      out.write("                    <a href=\"addunit.jsp\">Add Unit</a>\n");
      out.write("                </li>\n");
      out.write("                <li class=\"subitem2\">\n");
      out.write("                    <a href=\"addconsultingroom.jsp\">Add Consulting Room</a>\n");
      out.write("                </li>\n");
      out.write("                <li class=\"subitem3\">\n");
      out.write("                    <a href=\"addconcharges.jsp\">Add Consultation Type</a>\n");
      out.write("                </li>\n");
      out.write("                <li class=\"subitem4\">\n");
      out.write("                    <a href=\"addward.jsp\">Add Ward</a>\n");
      out.write("                </li>\n");
      out.write("                <li class=\"subitem4\">\n");
      out.write("                    <a href=\"addsponsor.jsp\">Add Sponsor</a>\n");
      out.write("                </li>\n");
      out.write("                <li class=\"subitem4\">\n");
      out.write("                    <a href=\"addstaff.jsp\">Add Staff</a>\n");
      out.write("                </li>\n");
      out.write("                <li class=\"subitem4\">\n");
      out.write("                    <a href=\"addrole.jsp\">Add Role</a>\n");
      out.write("                </li>\n");
      out.write("                <li class=\"subitem4\">\n");
      out.write("                    <a href=\"addconitems.jsp\">Settings</a>\n");
      out.write("                </li>\n");
      out.write("            </ul>\n");
      out.write("        </li>\n");
      out.write("\n");
      out.write("    </ul>\n");
      out.write("</div>");
      out.write("\n");
      out.write("\n");
      out.write("                    <div style=\"margin-top: 0px; \"class=\"span12 offset3 content1 hide\">\n");
      out.write("\n");
      out.write("                        <div style=\"padding-bottom: 80px;\" class=\"span9 thumbnail well content\">\n");
      out.write("                            <ul style=\"margin-left: 0px;\" class=\"breadcrumb\">\n");
      out.write("                                <li>\n");
      out.write("                                    <a> <i class=\"icon-tasks\"></i> Accounts</a><br/>\n");
      out.write("                                    ");
if (session.getAttribute("lasterror") != null) {
      out.write("\n");
      out.write("                                    <h3><a> ");
      out.print(session.getAttribute("lasterror"));
      out.write(" </a></h3>\n");
      out.write("                                    ");
}
      out.write("\n");
      out.write("                                </li>\n");
      out.write("\n");
      out.write("                            </ul>\n");
      out.write("                            <table class=\"example display\">\n");
      out.write("                                <thead>\n");
      out.write("                                    <tr>\n");
      out.write("                                        <th> Patient No </th>\n");
      out.write("                                        <th> Patient Name </th>\n");
      out.write("                                        <th> Date of Birth  </th>\n");
      out.write("                                        <th> Sponsor </th>\n");
      out.write("                                        <th> Membership ID </th>\n");
      out.write("                                        <th> Request Date </th>\n");
      out.write("                                        <th></th>\n");
      out.write("                                    </tr>\n");
      out.write("                                </thead>\n");
      out.write("                                <tbody>\n");
      out.write("                                    ");
 for (int i = 0; i < visits.size(); i++) {
                                            Visitationtable vst = (Visitationtable) visits.get(i);
                                    
      out.write("\n");
      out.write("                                    <tr>\n");
      out.write("                                        <td colspan=\"7\">\n");
      out.write("                                            <div class=\"dialog\" id=\"");
      out.print(vst.getVisitid());
      out.write("_dialog\" title=\"Accounts\">\n");
      out.write("\n");
      out.write("                                                <div style=\"margin-left: -50px;\" class=\"span2 \">\n");
      out.write("                                                    <ul class=\"menu\">\n");
      out.write("                                                        <li>  <a class=\"history_link \"> <i class=\"icon icon-white icon-list-alt\"> </i>  History </a></li>\n");
      out.write("                                                        <li> <a class=\"vital_link active\"> <i class=\"icon-white icon-list-alt\"> </i> Vitals </a></li>\n");
      out.write("                                                        <li> <a class=\"diagnosis_link\"> <i class=\"icon-white icon-list-alt\"> </i> Diagnosis </a></li>\n");
      out.write("                                                        <li> <a class=\"laboratory_link\"> <i class=\"icon-white icon-list-alt\"> </i> Lab Request </a></li>\n");
      out.write("                                                        <li> <a class=\"results_link\"> <i class=\"icon-white icon-list-alt\"> </i> Lab Results </a></li>\n");
      out.write("                                                        <li> <a class=\"prescription_link\"> <i class=\"icon-white icon-list-alt\"> </i> Prescription </a></li>\n");
      out.write("                                                    </ul>\n");
      out.write("                                                </div>\n");
      out.write("\n");
      out.write("                                                <div class=\"well thumbnail span8 pull-right\">\n");
      out.write("\n");
      out.write("\n");
      out.write("\n");
      out.write("\n");
      out.write("                                                    <ul style=\"margin-left: 0px;\" class=\"breadcrumb\">\n");
      out.write("                                                        <li>\n");
      out.write("                                                            <span class=\"divider\"></span> Patient Receipt\n");
      out.write("                                                        </li>    \n");
      out.write("                                                    </ul>\n");
      out.write("                                                    <form action=\"action/accountsaction.jsp\" method=\"post\">\n");
      out.write("                                                        <table class=\"table example display\">\n");
      out.write("                                                            <thead>\n");
      out.write("                                                                <tr style=\"color: #000;\">\n");
      out.write("                                                                    <th> Item </th>\n");
      out.write("                                                                    <th> Unit Cost </th>\n");
      out.write("                                                                    <th>Quantity </th>\n");
      out.write("                                                                    <th> Total </th>\n");
      out.write("\n");
      out.write("                                                                </tr>\n");
      out.write("                                                            </thead>\n");
      out.write("                                                            <tbody>\n");
      out.write("                                                                ");

                                                                    Double total = 0.0;
                                                                    List ptreatmentss = mgr.patientTreatment(vst.getVisitid());
                                                                    List pinvestigations = mgr.patientInvestigation(vst.getVisitid());
                                                                    for (int r = 0; r < ptreatmentss.size(); r++) {
                                                                        Patienttreatment ptPatienttreatments = (Patienttreatment) ptreatmentss.get(r);
                                                                        if (ptPatienttreatments.getDispensed().equalsIgnoreCase("No")) {
                                                                
      out.write("\n");
      out.write("                                                                <tr>\n");
      out.write("                                                                    <td class=\"patient\" rel=\"popover\" data-original-title=\"<span style='text-align:center;'> <h3>Patient Information Summary </h3> <h5>");
      out.print(mgr.getPatientByID(vst.getPatientid()).getFname());
      out.write(' ');
      out.print(mgr.getPatientByID(vst.getPatientid()).getLname());
      out.write("</h5> <h5><b> Date of Birth :</b> ");
      out.print(mgr.getPatientByID(vst.getPatientid()).getDateofbirth());
      out.write("</h5> </span>\"\n");
      out.write("                                                                        data-content=\"<table class='table table-bordered'> <tr> <td> Gender  </td> <td> ");
      out.print(mgr.getPatientByID(vst.getPatientid()).getGender());
      out.write(" </td> </tr> <tr> <td> Employer </td> <td> ");
      out.print(mgr.getPatientByID(vst.getPatientid()).getEmployer());
      out.write(" </td>  </tr> <tr> <td> Sponsor </td> <td> ");
      out.print(mgr.getSponsor(mgr.sponsorshipDetails(vst.getPatientid()).getSponsorid()).getSponsorname());
      out.write("</td> </tr> <tr>\n");
      out.write("                                                                        <td> Policy Number </td> <td> ");
      out.print(mgr.sponsorshipDetails(vst.getPatientid()).getMembershipid());
      out.write(" </td> </tr> <tr> <td> Benefit Plan </td> <td> ");
      out.print(mgr.sponsorshipDetails(vst.getPatientid()).getBenefitplan());
      out.write(" </td> </tr>  </table> \">\n");
      out.write("                                                                        ");
      out.print(mgr.getTreatment(ptPatienttreatments.getTreatmentid()).getTreatment());
      out.write(" </td>\n");
      out.write("\n");
      out.write("                                                                    <td>");
      out.print(ptPatienttreatments.getPrice());
      out.write(" </td>\n");
      out.write("                                                                    <td>");
      out.print(ptPatienttreatments.getQuantity());
      out.write(" </td>\n");
      out.write("                                                                    <td>");
      out.print( (ptPatienttreatments.getQuantity() * ptPatienttreatments.getPrice()));
      out.write("</td>\n");
      out.write("                                                            <input type=\"hidden\" name=\"pid[]\" value=\"");
      out.print(ptPatienttreatments.getId());
      out.write("\"/>\n");
      out.write("                                                            ");
 total = total + (ptPatienttreatments.getQuantity() * ptPatienttreatments.getPrice());
      out.write("\n");
      out.write("                                                            </tr>\n");
      out.write("\n");
      out.write("                                                            ");
}
                                                                }
                                                                for (int r = 0; r < pinvestigations.size(); r++) {
                                                                    Patientinvestigation patientinvestigation = (Patientinvestigation) pinvestigations.get(r);

                                                                    if (patientinvestigation.getPerformed().equalsIgnoreCase("No")) {

                                                            
      out.write("\n");
      out.write("                                                            <tr>\n");
      out.write("                                                            <input type=\"hidden\" name=\"vid[]\" value=\"");
      out.print(patientinvestigation.getId());
      out.write("\"/>\n");
      out.write("                                                            <td class=\"patient\" rel=\"popover\" data-original-title=\"<span style='text-align:center;'> <h3>Patient Information Summary </h3> <h5>");
      out.print(mgr.getPatientByID(vst.getPatientid()).getFname());
      out.write(' ');
      out.print(mgr.getPatientByID(vst.getPatientid()).getLname());
      out.write("</h5> <h5><b> Date of Birth :</b> ");
      out.print(mgr.getPatientByID(vst.getPatientid()).getDateofbirth());
      out.write("</h5> </span>\"\n");
      out.write("                                                                data-content=\"<table class='table table-bordered'> <tr> <td> Gender  </td> <td> ");
      out.print(mgr.getPatientByID(vst.getPatientid()).getGender());
      out.write(" </td> </tr> <tr> <td> Employer </td> <td> ");
      out.print(mgr.getPatientByID(vst.getPatientid()).getEmployer());
      out.write(" </td>  </tr> <tr> <td> Sponsor </td> <td> ");
      out.print(mgr.getSponsor(mgr.sponsorshipDetails(vst.getPatientid()).getSponsorid()).getSponsorname());
      out.write("</td> </tr> <tr>\n");
      out.write("                                                                <td> Policy Number </td> <td> ");
      out.print(mgr.sponsorshipDetails(vst.getPatientid()).getMembershipid());
      out.write(" </td> </tr> <tr> <td> Benefit Plan </td> <td> ");
      out.print(mgr.sponsorshipDetails(vst.getPatientid()).getBenefitplan());
      out.write(" </td> </tr>  </table> \">\n");
      out.write("                                                                ");
      out.print(mgr.getInvestigation(patientinvestigation.getInvestigationid()).getInvestigation());
      out.write(" </td>\n");
      out.write("                                                            <td>");
      out.print(patientinvestigation.getPrice());
      out.write("</td>\n");
      out.write("                                                            <td>Once </td>\n");
      out.write("                                                            <td>");
      out.print(patientinvestigation.getPrice());
      out.write("</td>\n");
      out.write("\n");
      out.write("                                                            ");
 total = total + patientinvestigation.getPrice();
      out.write(" \n");
      out.write("\n");
      out.write("                                                            </tr>\n");
      out.write("\n");
      out.write("                                                            ");
}
                                                                    }
      out.write(" \n");
      out.write("                                                            <tr>\n");
      out.write("                                                                <td>");
      out.print(mgr.getConsultationId(vst.getVisittype()).getContype());
      out.write("</td>\n");
      out.write("                                                                <td></td>\n");
      out.write("                                                                <td></td>\n");
      out.write("                                                                <td>");
      out.print(mgr.getConsultationId(vst.getVisittype()).getAmount());
      out.write("</td>\n");
      out.write("                                                            </tr>\n");
      out.write("                                                            <tr>\n");
      out.write("                                                                <td>Total Cost</td>\n");
      out.write("                                                                <td></td>\n");
      out.write("                                                                <td></td>\n");
      out.write("                                                                <td>");
      out.print(total + mgr.getConsultationId(vst.getVisittype()).getAmount());
      out.write("</td>\n");
      out.write("                                                            </tr>\n");
      out.write("\n");
      out.write("                                                            </tbody>\n");
      out.write("\n");
      out.write("                                                        </table>\n");
      out.write("\n");
      out.write("                                                        <div style=\"text-align: center;\" class=\"form-actions\">\n");
      out.write("\n");
      out.write("                                                            <!-- <input type=\"hidden\" name=\"unitid\" value=\"Accounts\"/>-->\n");
      out.write("                                                            <input type=\"hidden\" name=\"patient\" value=\"");
      out.print(vst.getPatientid());
      out.write("\"/>\n");
      out.write("                                                            <input type=\"hidden\" name=\"visitid\" value=\"");
      out.print(vst.getVisitid());
      out.write("\"/>\n");
      out.write("                                                            <!-- <input type=\"submit\" name=\"action\" value=\"Forward to Accounts\"/>-->\n");
      out.write("                                                            <select name=\"unitid\">\n");
      out.write("                                                                ");

                                                                    List units = mgr.listUnits();
                                                                    for (int j = 0; j < units.size(); j++) {
                                                                        Units unit = (Units) units.get(j);
                                                                
      out.write("\n");
      out.write("                                                                <option value=\"");
      out.print(unit.getUnitname());
      out.write('"');
      out.write('>');
      out.print(unit.getUnitname());
      out.write("</option> \n");
      out.write("                                                                ");
 }
                                                                    List wards = mgr.listWard();
                                                                    for (int j = 0; j < wards.size(); j++) {
                                                                        Ward ward = (Ward) wards.get(j);
                                                                
      out.write("\n");
      out.write("                                                                <option value=\"");
      out.print(ward.getWardname());
      out.write('"');
      out.write('>');
      out.print(ward.getWardname());
      out.write("</option> \n");
      out.write("                                                                ");
 }

                                                                
      out.write("\n");
      out.write("                                                            </select>\n");
      out.write("                                                            <br/>\n");
      out.write("\n");
      out.write("                                                            <button type=\"submit\" name=\"action\" value=\"Generate Receipt\" class=\"btn btn-danger btn-large\">\n");
      out.write("\n");
      out.write("                                                                <i class=\"icon-white icon-arrow-right\"> </i> Forward\n");
      out.write("                                                            </button>\n");
      out.write("                                                        </div>\n");
      out.write("                                                    </form>\n");
      out.write("                                                </div>\n");
      out.write("                                            </div>\n");
      out.write("                                        </td>\n");
      out.write("                                    </tr>\n");
      out.write("\n");
      out.write("                                    <tr>\n");
      out.write("                                        <td class=\"patient\" rel=\"popover\" data-original-title=\"<span style='text-align:center;'> <h3>Patient Information Summary </h3> <h5>");
      out.print(mgr.getPatientByID(vst.getPatientid()).getFname());
      out.write(' ');
      out.print(mgr.getPatientByID(vst.getPatientid()).getLname());
      out.write("</h5> <h5><b> Date of Birth :</b> ");
      out.print(mgr.getPatientByID(vst.getPatientid()).getDateofbirth());
      out.write("</h5> </span>\"\n");
      out.write("                                            data-content=\"<table class='table table-bordered'> <tr> <td> Gender  </td> <td> ");
      out.print(mgr.getPatientByID(vst.getPatientid()).getGender());
      out.write(" </td> </tr> <tr> <td> Employer </td> <td> ");
      out.print(mgr.getPatientByID(vst.getPatientid()).getEmployer());
      out.write(" </td>  </tr> <tr> <td> Sponsor </td> <td> ");
      out.print(mgr.getSponsor(mgr.sponsorshipDetails(vst.getPatientid()).getSponsorid()).getSponsorname());
      out.write("</td> </tr> <tr>\n");
      out.write("                                            <td> Policy Number </td> <td> ");
      out.print(mgr.sponsorshipDetails(vst.getPatientid()).getMembershipid());
      out.write(" </td> </tr> <tr> <td> Benefit Plan </td> <td> ");
      out.print(mgr.sponsorshipDetails(vst.getPatientid()).getBenefitplan());
      out.write(" </td> </tr>  </table> \"> ");
      out.print(vst.getPatientid());
      out.write("   </td>\n");
      out.write("                                        <td>");
      out.print(mgr.getPatientByID(vst.getPatientid()).getFname());
      out.write(' ');
      out.print(mgr.getPatientByID(vst.getPatientid()).getLname());
      out.write("</td>\n");
      out.write("                                        <td>");
      out.print(mgr.getPatientByID(vst.getPatientid()).getDateofbirth());
      out.write(" </td>\n");
      out.write("\n");
      out.write("                                        <td>");
      out.print(mgr.getSponsor(mgr.sponsorshipDetails(vst.getPatientid()).getSponsorid()).getSponsorname());
      out.write(" </td>\n");
      out.write("                                        <td>");
      out.print(mgr.sponsorshipDetails(vst.getPatientid()).getMembershipid());
      out.write("   </td>\n");
      out.write("\n");
      out.write("                                        <td>");
      out.print(vst.getDate());
      out.write(" </td>\n");
      out.write("\n");
      out.write("                                        <td>\n");
      out.write("                                            <button class=\"btn btn-info\" id=\"");
      out.print(vst.getVisitid());
      out.write("_link\">\n");
      out.write("                                                <i class=\"icon-white icon-check\"></i> Receipt\n");
      out.write("                                            </button></td>\n");
      out.write("                                    </tr>\n");
      out.write("                                    ");
}
      out.write(" \n");
      out.write("\n");
      out.write("                                </tbody>\n");
      out.write("                            </table>\n");
      out.write("\n");
      out.write("                        </div>\n");
      out.write("\n");
      out.write("                    </div>\n");
      out.write("                    <div class=\"clearfix\"></div>\n");
      out.write("\n");
      out.write("                </div>\n");
      out.write("\n");
      out.write("            </section>\n");
      out.write("\n");
      out.write("            <footer style=\"display: none; margin-top: 50px;\" class=\"footer\">\n");
      out.write("                <p style=\"text-align: center\" class=\"pull-right\">\n");
      out.write("                    <img src=\"images/logo.png\" width=\"100px;\" />\n");
      out.write("                </p>\n");
      out.write("            </footer>\n");
      out.write("\n");
      out.write("        </div><!-- /container -->    \n");
      out.write("\n");
      out.write("    </div>\n");
      out.write("</div>\n");
      out.write("<!--end static dialog-->\n");
      out.write("\n");
      out.write("<!-- Le javascript\n");
      out.write("================================================== -->\n");
      out.write("<!-- Placed at the end of the document so the pages load faster -->\n");
      out.write("<script src=\"js/jquery.js\"></script>\n");
      out.write("<script src=\"js/bootstrap-dropdown.js\"></script>\n");
      out.write("<script src=\"js/bootstrap-scrollspy.js\"></script>\n");
      out.write("<script src=\"js/bootstrap-collapse.js\"></script>\n");
      out.write("<script src=\"js/bootstrap-tooltip.js\"></script>\n");
      out.write("<script src=\"js/bootstrap-popover.js\"></script>\n");
      out.write("<script src=\"js/application.js\"></script>\n");
      out.write("\n");
      out.write("<script type=\"text/javascript\" src=\"js/jquery-ui-1.8.16.custom.min.js\"></script>\n");
      out.write("\n");
      out.write("<script type=\"text/javascript\" src=\"third-party/jQuery-UI-Date-Range-Picker/js/date.js\"></script>\n");
      out.write("<script type=\"text/javascript\" src=\"third-party/jQuery-UI-Date-Range-Picker/js/daterangepicker.jQuery.js\"></script>\n");
      out.write("\n");
      out.write("<script src=\"third-party/wijmo/jquery.mousewheel.min.js\" type=\"text/javascript\"></script>\n");
      out.write("<script src=\"third-party/wijmo/jquery.bgiframe-2.1.3-pre.js\" type=\"text/javascript\"></script>\n");
      out.write("<script src=\"third-party/wijmo/jquery.wijmo-open.1.5.0.min.js\" type=\"text/javascript\"></script>\n");
      out.write("\n");
      out.write("<script src=\"third-party/jQuery-UI-FileInput/js/enhance.min.js\" type=\"text/javascript\"></script>\n");
      out.write("<script src=\"third-party/jQuery-UI-FileInput/js/fileinput.jquery.js\" type=\"text/javascript\"></script>\n");
      out.write("\n");
      out.write("<script type=\"text/javascript\" src=\"js/tablecloth.js\"></script>\n");
      out.write("<script type=\"text/javascript\" src=\"js/jquery.dataTables.js\"></script>\n");
      out.write("<script type=\"text/javascript\" src=\"js/demo.js\"></script>\n");
      out.write("\n");
      out.write("<!--initiate accordion-->\n");
      out.write("<script type=\"text/javascript\">\n");
      out.write("    $(function() {\n");
      out.write("\n");
      out.write("        var menu_ul = $('.menu > li > ul'), menu_a = $('.menu > li > a');\n");
      out.write("\n");
      out.write("        menu_ul.hide();\n");
      out.write("\n");
      out.write("        $(\".menu\").fadeIn();\n");
      out.write("        $(\".content1\").fadeIn();\n");
      out.write("        $(\".navbar\").fadeIn();\n");
      out.write("        $(\".footer\").fadeIn();\n");
      out.write("        $(\".subnav\").fadeIn();\n");
      out.write("        $(\".progress1\").hide();\n");
      out.write("\n");
      out.write("        menu_a.click(function(e) {\n");
      out.write("            e.preventDefault();\n");
      out.write("            if(!$(this).hasClass('active')) {\n");
      out.write("                menu_a.removeClass('active');\n");
      out.write("                menu_ul.filter(':visible').slideUp('normal');\n");
      out.write("                $(this).addClass('active').next().stop(true, true).slideDown('normal');\n");
      out.write("            } else {\n");
      out.write("                $(this).removeClass('active');\n");
      out.write("                $(this).next().stop(true, true).slideUp('normal');\n");
      out.write("            }\n");
      out.write("        });\n");
      out.write("\n");
      out.write("    });\n");
      out.write("\n");
      out.write("</script>\n");
      out.write("\n");
      out.write("\n");
      out.write("<script type=\"text/javascript\" charset=\"utf-8\">\n");
      out.write("    $(document).ready(function() {\n");
      out.write("        $('.example').dataTable({\n");
      out.write("            \"bJQueryUI\" : true,\n");
      out.write("            \"sPaginationType\" : \"full_numbers\",\n");
      out.write("            \"iDisplayLength\" : 25,\n");
      out.write("            \"aaSorting\" : [],\n");
      out.write("            \"bSort\" : true\n");
      out.write("\n");
      out.write("        });\n");
      out.write("        $(\".patient\").popover({\n");
      out.write("\n");
      out.write("            placement : 'right',\n");
      out.write("            animation : true\n");
      out.write("\n");
      out.write("        });\n");
      out.write("\n");
      out.write("    });\n");
      out.write("\n");
      out.write("</script>\n");
 for (int i = 0;
            i < visits.size();
            i++) {
        Visitationtable vst = (Visitationtable) visits.get(i);

      out.write("\n");
      out.write("\n");
      out.write("\n");
      out.write("<script type=\"text/javascript\">\n");
      out.write("   \n");
      out.write("                      \n");
      out.write("    $(\"#");
      out.print( vst.getVisitid());
      out.write("_dialog\").dialog({\n");
      out.write("        autoOpen : false,\n");
      out.write("        width : 1000,\n");
      out.write("        modal :true,\n");
      out.write("        position : 'top'\n");
      out.write("\n");
      out.write("    });\n");
      out.write("    \n");
      out.write("    $(\"#");
      out.print( vst.getVisitid());
      out.write("_adddrug_dialog\").dialog({\n");
      out.write("        autoOpen : false,\n");
      out.write("        width : 1000,\n");
      out.write("        modal :true\n");
      out.write("\n");
      out.write("    });\n");
      out.write("    \n");
      out.write("   \n");
      out.write("    \n");
      out.write("    $(\"#");
      out.print( vst.getVisitid());
      out.write("_link\").click(function(){\n");
      out.write("      \n");
      out.write("        $(\"#");
      out.print(vst.getVisitid());
      out.write("_dialog\").dialog('open');\n");
      out.write("    \n");
      out.write("    })\n");
      out.write("   \n");
      out.write("    $(\"#");
      out.print( vst.getPatientid());
      out.write("_dialog\").dialog({\n");
      out.write("        autoOpen : false,\n");
      out.write("        width : 1000,\n");
      out.write("        modal :true\n");
      out.write("\n");
      out.write("    });\n");
      out.write("    \n");
      out.write("   \n");
      out.write("    \n");
      out.write("    $(\"#");
      out.print( vst.getVisitid());
      out.write("_adddrug_link\").click(function(){\n");
      out.write("        alert(\"\");\n");
      out.write("        $(\"#");
      out.print(vst.getVisitid());
      out.write("_adddrug_dialog\").dialog('open');\n");
      out.write("        \n");
      out.write("    })\n");
      out.write("   \n");
      out.write("    \n");
      out.write("</script>\n");
      out.write("\n");
      out.write("\n");
 }
      out.write("\n");
      out.write("\n");
      out.write("</body>\n");
      out.write("</html>\n");
    } catch (Throwable t) {
      if (!(t instanceof SkipPageException)){
        out = _jspx_out;
        if (out != null && out.getBufferSize() != 0)
          out.clearBuffer();
        if (_jspx_page_context != null) _jspx_page_context.handlePageException(t);
        else throw new ServletException(t);
      }
    } finally {
      _jspxFactory.releasePageContext(_jspx_page_context);
    }
  }
}
